...
public class MovieParkApplication {
    public static void main(String[] args) {
        if(args.length > 0 && args[0] != null) {
            ...
            initCustomApplicationProperties(Paths.get("application.yaml"));
        }
    ...
    private static void initCustomApplicationProperties(Path filePath) {
        try {
            log.info("Custom application file path: {}", filePath);
            log.info("Custom application file exists: {}", filePath.toFile().exists());
            if (filePath.toFile().exists()) {
                log.info("Reading custom application file:\n{}", new String(Files.readAllBytes(filePath)));
